...
namespace TrendMeEditor
{
    ...
    [CustomPropertyDrawer(typeof(RemoteObservableField))]
    public class RemoteObservableFieldDrawer : RemoteFieldDrawer<IObservable<object>>
    {
        protected override string PrintValue(SerializedProperty property)
        {
            return property.FindPropertyRelative("_Value").objectReferenceValue.ToString();
        }
    }
    ...
    public abstract class RemoteFieldDrawer<T> : PropertyDrawer
    {
        ...
        {
            BindingFlags bindingFlags = (BindingFlags) property.FindPropertyRelative("bindingFlags").enumValueIndex;
            ...
            for (int i = 0; i < splitstring.Length; i++)
            {
                ...
                options.Add("");
                MemberInfo[] members = currentType.GetMembers(bindingFlags);
                ...
                if (options.Count > 1)
                {
                    ...
                    MemberInfo[] info = currentType.GetMember(
                        options[selectedIndexs[i + 1]], bindingFlags);
                    if (info.Length > 0)
                    {
            ...
            if (currentType != typeof(T) && !CheckGenericAndAssignable(currentType, typeof(T)))
            {
            ...
            else
            {
                position.y += lineHeight + 1;
                ;
                EditorGUI.LabelField(position, "Correct value");
            }
            property.FindPropertyRelative("fieldName").stringValue = fieldName;
        }

        private bool CheckGenericAndAssignable(Type currentType, Type assignableType)
        {
            if (!currentType.IsGenericType || !assignableType.IsGenericType) return false;

            return currentType.GetGenericTypeDefinition() == assignableType.GetGenericTypeDefinition() &&
                assignableType.GetGenericArguments().Select((type, index) => new { type, index })
                    .All(x => x.type.IsAssignableFrom(currentType.GetGenericArguments()[x.index]));
        }
